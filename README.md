...
There are early HTTP routes, a Vite client that calls `/api` (proxied in dev) with hash routes (`#/`, `#/chat`), and a WebSocket the chat UI uses when you’re signed in for new messages. The dev server proxies `/ws` alongside `/api` and `/uploads`, so the client can speak same-origin WebSockets without extra configuration. Storage is in-memory for now, not a real database yet.
